...
const inputTypeOptions: string[] = ['string', 'number', 'boolean', 'method', 'regexp', 'integer', 'float', 'object', 'enum', 'date', 'url', 'hex', 'email'];
...
function prepareDynamicFormComponents(): React.FC<DynamicCompProps>[] {
  const forms = [
    {title: "Contact Details", fields: [{field: "First Name", type: "text"}, {field: "Email", type: "email"}]},
    {title: "Agreement", fields: [{field: "Parent Compnay", type: "text"}]},
    {title: "Feedback", fields: [{field: "Feedback", type: "text"}]}
  ]
      ...
      return <>
        <Form title={item.title} form={antForm}>
          ...
          {item.fields?.map((field: FormField, fieldIndex: number) => {
            const rule: any = {
              ...(inputTypeOptions.includes(field.type) && {type: field.type}),
              message: `Please enter valid value for ${field.field}`,
            }
            return <Form.Item key={fieldIndex} name={field.field} label={field.field} rules={[rule]}>
...
const PackagesList: React.FC<any> = () => {
  return <div><h2>--Packages--</h2></div>
}

const Summary: React.FC<any> = () => {
  return <div><h2>--Summary--</h2></div>
}
...
function App() {
  ...
  const formSections = dynamicFormComponents.map((Component: React.FC<DynamicCompProps>) => {
